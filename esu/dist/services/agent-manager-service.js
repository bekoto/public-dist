        ...
        (0, class_validator_1.validate)(data).then(e => {
            if (e && e.length > 0) {
                ...
                this.configServ.getEtablissement(data.idEtabilissement).subscribe(et => {
                    ...
                    this.configServ.getCentre(data.idCentre).subscribe(c => {
                        agentToSave.centre = c;
                        new this.agentModel(agentToSave).save().then(a => {
                            if (a) {
                                ...
                                form.app = (0, jwt_decode_1.default)(appToken).app;
                                form.app = data.app;
                                form.nom = data.nom;
                                ...
                                (0, jwt_decode_1.default)(appToken);
                                this.httpService.post(app_config_1.USER_MANAGER_API_URL + "/users", form, {
                                    headers: {
                                        "Authorization": `Bearer ${appToken.replace('Bearer ', '')}`
                                    }
                                }).subscribe(d => {
                                    let u = d.data;
