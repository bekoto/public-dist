...
let ConfigService = class ConfigService {
    ...
    addProvince(data) {
        ...
        (0, class_validator_1.validate)(data).then(eInp => {
            if (eInp && eInp.length > 0) {
                s.error(new common_1.HttpException('Bad data', common_1.HttpStatus.BAD_REQUEST));
            }
            else {
                new this.provinceModel(data).save().then(p => {
                    if (!p) {
    ...
    getProvince(idprovince) {
        let s = new rxjs_1.Subject();
        this.provinceModel.findById(idprovince).populate(["villes", "territoires"]).exec().then(p => {
            if (!p) {
                s.error(new common_1.HttpException('Provice not found', common_1.HttpStatus.NOT_FOUND));
            }
            else {
                s.next(p);
                s.complete();
    ...
    getProvinces() {
        return rxjs_1.Observable.create(s => {
            this.provinceModel.find().exec().then(p => {
                if (!p) {
                    s.error(new common_1.HttpException('Provice not found', common_1.HttpStatus.NOT_FOUND));
                }
                else {
    ...
    getCentre(idcentre) {
        let s = new rxjs_1.Subject();
        this.centreModel.findById(idcentre).exec().then(c => {
            if (!s) {
                s.error(new common_1.HttpException('Centre not found', common_1.HttpStatus.NOT_FOUND));
            }
            else {
                console.debug("cenfff", c);
                s.next(c);
                s.complete();
            }
        });
    ...
    getEtablissementByCode(code) {
        ...
        this.etablissementModel.findOne({
            ...
            console.debug("ddeee", et.id.toString());
            s.next(et);
            s.complete();
        }).catch(e => {
            console.log(e);
