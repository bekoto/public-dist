...
let AuthService = class AuthService {
    ...
    login(auth) {
        ...
        (0, class_validator_1.validate)(auth).then(d => {
            console.log("oooauth", auth);
            this.userMan.findByUsernameOrPhoneOrEmailAndApp(auth.username, auth.username, auth.username, auth.app).subscribe(u => {
                console.log("uuf", u);
                if (!u) {
                    ...
                    if (u.isAllowed == false) {
                        s.error(new common_1.HttpException('Account not activated', common_1.HttpStatus.UNAUTHORIZED));
                    }
                    else {
                        const o = u;
