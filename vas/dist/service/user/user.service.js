...
const privileges_1 = require("../../model/privileges");
let UserService = class UserService {
    ...
    findModuleOfRole(module) {
        ...
        this.appModel.aggregate([
            {
                $match: {
                    "modules.id": new mongoose_2.default.Schema.Types.ObjectId(module)
                },
            }, {
                $project: {
                    "modules": 1
            ...
            $and: [
                {
                    $or: [
                        ...
                        {
                            id: new mongoose_2.default.Schema.Types.ObjectId(app)
                        }
                    ]
                },
                {
            ...
            $and: [
                ...
                },
                {
                    $or: [
                        ...
                        {
                            'application.id': new mongoose_2.default.Types.ObjectId(app)
                        },
                    ]
                },
            ]
            ...
            return this.connection.startSession().then(cs => {
                ...
                this.roleModel.find({
                    ...
                    this.moduleModel.find({
                        ...
                        let roleAdmin = rs[0];
                        for (const m of modf) {
                            let mr = new role_module_1.ModulePrivileges();
                            mr.module = m;
                            mr.privileges = [privileges_1.Privilege.ALL];
                            roleAdmin.modules.push(mr);
                        }
                        new this.appModel(app).save().then(async (a) => {
                            let u = new user_1.User();
                            const salt = bcrypt.genSaltSync(10);
                            u.roles.push(roleAdmin);
                            u.email = data.email;
                            u.nom = data.nom;
                            u.prenom = data.prenom;
                            u.code = util_1.Util.generateOTP(20);
    ...
    findByUsernameAndApp(username, app) {
        ...
        this.userrModel.findOne({
            $and: [
                ...
                {
                    application: new mongoose_2.default.Schema.Types.ObjectId(app)
                }
            ]
        }).populate(['application', 'category', 'roles']).exec().then(d => {
            u.next(d);
